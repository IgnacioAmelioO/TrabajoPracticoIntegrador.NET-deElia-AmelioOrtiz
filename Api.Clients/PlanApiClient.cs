    ...
    public class PlanApiClient
    {
        ...
        public static async Task UpdateAsync(PlanDTO plan)
        {
            try
            {
                ...
                {
                    string errorContent = await response.Content.ReadAsStringAsync();
                    throw new Exception($"Error al actualizar plan con Id {plan.Id_plan}. Status: {response.StatusCode}, Detalle: {errorContent}");
                }
            }
            catch (HttpRequestException ex)
            {
                throw new Exception($"Error de conexión al actualizar plan con Id {plan.Id_plan}: {ex.Message}", ex);
            }
            catch (TaskCanceledException ex)
            {
                throw new Exception($"Timeout al actualizar plan con Id {plan.Id_plan}: {ex.Message}", ex);
            }
        }
        ...
        public static async Task<IEnumerable<PlanDTO>> GetByCriteriaAsync(string texto)
        {
            try
            {
                HttpResponseMessage response = await client.GetAsync($"planes/criteria?texto={Uri.EscapeDataString(texto)}");

                if (response.IsSuccessStatusCode)
                {
                    var planes = await response.Content.ReadFromJsonAsync<IEnumerable<PlanDTO>>();
                    return planes ?? new List<PlanDTO>();
                }
                else
                {
                    string errorContent = await response.Content.ReadAsStringAsync();
                    throw new Exception($"Error al buscar planes. Status: {response.StatusCode}, Detalle: {errorContent}");
                }
            }
            catch (HttpRequestException ex)
            {
