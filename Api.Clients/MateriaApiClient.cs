...
namespace Api.Clients
{
    public class MateriaApiClient
    {
        ...
        public static async Task<MateriaDTO> GetAsync(int id)
        {
            ...
            {
                throw new Exception($"Error de conexión al obtener materia con Id {id}: {ex.Message}", ex);
            }
        ...
        public static async Task AddAsync(MateriaDTO materia)
        {
            try
            {
        ...
        public static async Task UpdateAsync(MateriaDTO materia)
        {
            try
            {
                ...
                {
                    string errorContent = await response.Content.ReadAsStringAsync();
                    throw new Exception($"Error al actualizar materia con Id {materia.Id_materia}. Status: {response.StatusCode}, Detalle: {errorContent}");
                }
            }
            catch (HttpRequestException ex)
            {
                throw new Exception($"Error de conexión al actualizar materia con Id {materia.Id_materia}: {ex.Message}", ex);
            }
            catch (TaskCanceledException ex)
            {
                throw new Exception($"Timeout al actualizar materia con Id {materia.Id_materia}: {ex.Message}", ex);
            }
        }
    }
}