    ...
    public class CursoApiClient
    {
        ...
        public static async Task UpdateAsync(CursoDTO curso)
        {
            try
            {
                ...
                {
                    string errorContent = await response.Content.ReadAsStringAsync();
                    throw new Exception($"Error al actualizar curso con Id {curso.Id_curso}. Status: {response.StatusCode}, Detalle: {errorContent}");
                }
            }
            catch (HttpRequestException ex)
            {
                throw new Exception($"Error de conexión al actualizar curso con Id {curso.Id_curso}: {ex.Message}", ex);
            }
            catch (TaskCanceledException ex)
            {
                throw new Exception($"Timeout al actualizar curso con Id {curso.Id_curso}: {ex.Message}", ex);
            }
        }
        ...
        public static async Task<IEnumerable<CursoDTO>> GetByCriteriaAsync(string texto)
        {
            try
            {
                HttpResponseMessage response = await client.GetAsync($"cursos/criteria?texto={Uri.EscapeDataString(texto)}");
