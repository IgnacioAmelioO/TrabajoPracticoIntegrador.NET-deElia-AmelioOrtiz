    ...
    public class EspecialidadApiClient
    {
        ...
        public static async Task UpdateAsync(EspecialidadDTO especialidad)
        {
            try
            {
                ...
                {
                    string errorContent = await response.Content.ReadAsStringAsync();
                    throw new Exception($"Error al actualizar especialidad con Id {especialidad.Id_especialidad}. Status: {response.StatusCode}, Detalle: {errorContent}");
                }
            }
            catch (HttpRequestException ex)
            {
                throw new Exception($"Error de conexión al actualizar especialidad con Id {especialidad.Id_especialidad}: {ex.Message}", ex);
            }
            catch (TaskCanceledException ex)
            {
                throw new Exception($"Timeout al actualizar especialidad con Id {especialidad.Id_especialidad}: {ex.Message}", ex);
            }
        }
        ...
        public static async Task<IEnumerable<EspecialidadDTO>> GetByCriteriaAsync(string texto)
        {
            try
            {
                HttpResponseMessage response = await client.GetAsync($"especialidades/criteria?texto={Uri.EscapeDataString(texto)}");

                if (response.IsSuccessStatusCode)
                {
                    var especialidades = await response.Content.ReadFromJsonAsync<IEnumerable<EspecialidadDTO>>();
                    return especialidades ?? new List<EspecialidadDTO>();
                }
                else
                {
                    string errorContent = await response.Content.ReadAsStringAsync();
                    throw new Exception($"Error al buscar especialidades. Status: {response.StatusCode}, Detalle: {errorContent}");
                }
            }
            catch (HttpRequestException ex)
            {
